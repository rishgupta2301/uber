...
const Home = () => {
  ...
  const fetchSuggestions = async (input, type) => {
    ...
    try {
      ...
      type === 'pickup' ? setPickupSuggestions(response.data) : setDestinationSuggestions(response.data);
    } catch (error) {
  ...
  // Debounced handlers for pickup and destination changes
  const debouncedFetchPickupSuggestions = useCallback(debounce((value) => fetchSuggestions(value, 'pickup'), 300), []);
  const debouncedFetchDestinationSuggestions = useCallback(debounce((value) => fetchSuggestions(value, 'destination'), 300), []);
